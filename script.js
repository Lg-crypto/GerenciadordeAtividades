...
const criarItemdeAtividade = (atividade)=>{
    ...
    const atv = `
        ...
        <div class="card-bg">
            ...
            <time class="full">${formatar.dia.semana.longo}, ${formatar.dia.numerico} de ${formatar.mes} às ${formatar.hora}h</time>
        </div>
    `;
...
const salvarAtividade = (event)=>{
    ...
    if(atividadeExiste){
        alert('Dia ou Hora não disponivel.')
        return 0;
    }
...
const criarDiasSelecao = ()=>{
    ...
    for (let dia of dias) {
        let formatar = formatador(dia);
        let formatarDia = formatar.dia.numerico + ' de ' + formatar.mes;

        diaSelecao +=`
        <option value="${dia}">${formatarDia}</option>
        `
    }
...
const criarHorasSelecao = ()=>{
    ...
    for (let i = 0; i < 24; i++) {
        ...
        horasDisponiveis += `<option value="${i}:00">${hora}:00</option>`;
        horasDisponiveis += `<option value="${i}:30">${hora}:30</option>`
    }
